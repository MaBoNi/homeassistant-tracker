...
def fetch_and_save_location(user_entity):
    ...
    logger.info(
        "Fetching location for %s from Home Assistant API: %s", user_entity, url
    )

    try:
        response = requests.get(url, headers=headers)
        logger.info(
            "Response Status Code for %s: %s", user_entity, response.status_code
        )
            ...
            for device_tracker in device_trackers:
                ...
                if device_response.status_code == 200:
                    ...
                    if latitude and longitude:
                        ...
                        logger.warning(
                            "No location data available for %s", device_tracker
                        )
                else:
                    logger.error(
